...
export const quanTriApi = {
  ...
  laySachBanChay: (params?: { tuNgay?: string; denNgay?: string; page?: number; size?: number }) =>
    goiApi<PhanTrang<SachBanChay>>({ url: '/api/quan-tri/thong-ke/sach-ban-chay', params: { size: 5, ...params } }),
  ...
  duyetDanhGia: (maDanhGia: number, phanHoi?: string) =>
    goiApi<DanhGia>({ url: `/api/quan-tri/danh-gia/${maDanhGia}/duyet`, method: 'PATCH', data: { phanHoi } }),
  tuChoiDanhGia: (maDanhGia: number, phanHoi?: string) =>
    goiApi<DanhGia>({ url: `/api/quan-tri/danh-gia/${maDanhGia}/tu-choi`, method: 'PATCH', data: { phanHoi } }),
